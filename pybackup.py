...
# References: http://boto.s3.amazonaws.com/s3_tut.html
# 
#
...
class Configuration:
		...
		try:
			...
			for line in lines:
				if line.startswith("# Version"):
					v = _get_second_token( line, ":")
					if v != "":
						retval.version = v
				elif line.startswith("#"):
					pass
				elif line.startswith("AWS_ACCESS_KEY"):
					retval.aws_access_key = _get_second_token( line, "=" )
				elif line.startswith("AWS_SECRET_KEY"):
					retval.aws_secret_key = _get_second_token( line, "=" )
				elif line.stsrtswith("S3_BUCKET"):
					retval.s3_bucket = _get_second_token( line, "=" )
					
					
				
		except IOError, err:
			retval.status = "error|{0}".format(err)
			
		return retval
		
	@staticmethod
	def generate_empty_config(filename, currentversion):
		FILE = open(filename, "w")
		FILE.writelines( Configuration.generate_config_header(currentversion) )
		FILE.close()

	@staticmethod
	def generate_config_header(version):
	...
	@staticmethod
	def _get_second_token(line, delimeter):
...
def read_configuration():
	...
	if os.path.exists(configfile):
		config = Configuration.load_from_file( configfile )
		return config
	else:
...
def configure():
	...
	try:	
		if not os.path.exists( config ):
			...
			Configuration.generate_empty_config( config, CURRENTVERSION )
